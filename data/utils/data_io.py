...
def load_patient_data_by_id(patient_id):
    return json.load(open('/media/kai/Shared Space/database-ICU/eICU/all/' + str(patient_id) + '.json'))


def load_processed_patient_data_by_id(patient_id):
    patient_data = pd.read_csv('processed_dataset/all/data/'+str(patient_id)+'.csv', sep='\t', header=0)
    patient_info = pd.read_csv('processed_dataset/all/info/'+str(patient_id)+'.csv', sep='\t', header=0)
